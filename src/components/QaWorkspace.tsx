...
export default function QaWorkspace() {
  ...
  return <section className="qa-workspace" aria-label="AI 问答工作区">
    ...
    <div className="qa-layout">
      ...
      <main className="qa-conversation">
        {!selected ? <div className="qa-empty"><strong>先选择一篇笔记</strong><span>只会把所选笔记的正文和转写作为问答依据。</span></div> : <>
          ...
          <div className="qa-composer"><label><span className="sr-only">向所选笔记提问</span><textarea aria-label="向所选笔记提问" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="向这篇笔记提问…" /></label><div className="qa-composer-actions"><button type="button" className="secondary-action" disabled={!question.trim() || !webReady || webSearching} title={webReady ? '从已配置的联网服务获取带来源结果' : '请先在设置的 AI 接入中启用并配置联网服务'} onClick={() => void searchWeb()}>{webSearching ? '检索中…' : '联网检索'}</button><button type="button" disabled={!question.trim() || sending} onClick={() => void send()}>{sending ? '发送中…' : '发送问题'}</button></div></div>
