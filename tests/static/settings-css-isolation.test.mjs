 ...
 * under .cipher-settings-root, preventing style leakage to non-settings pages.
 */
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (stripped.includes('{') && !stripped.startsWith('/*') && !stripped.startsWith('*')) {
    ...
    for (const sel of selectors) {
      ...
      if (!sel.startsWith('.cipher-settings-root')) {
        failures.push(`Line ${i + 1}: "${sel}" is not scoped under .cipher-settings-root`);
      }
    }
  }
}
...
if (failures.length > 0) {
  ...
  console.log('CSS isolation test PASSED: all selectors scoped under .cipher-settings-root');
}
